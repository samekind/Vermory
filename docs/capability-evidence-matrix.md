...
| `W38-official-openclaw-webchat` | `runtime-qualified` | Official OpenClaw Control UI passes real-model formation, explicit review, transcript-reset recall attribution, unrelated-session isolation, correction, current-only recall, forgetting, exact/paraphrased deletion probes, reload, service restart, and terminal operation replay at `28 / 28` | Real Chrome, OpenClaw `2026.6.11`, maintained plugin hooks, Grok CLI `0.2.111` with observed WebChat route `grok-4.5-build-free`, PostgreSQL 18.4 schema 25 | [Official OpenClaw WebChat](evidence/2026-07-23-official-openclaw-webchat.md) | Qualifies the exact UI and runtime trajectory, not model ranking, automatic governance, every browser, every OpenClaw release/channel, remote topology, or transcript erasure. |
...
| `W38-official-openclaw-webchat` | `runtime-qualified` | Official OpenClaw 2026.6.11 Control UI in real Chrome passes governed real-Grok formation, explicit accept/correct/forget, reset-attributed recall, unrelated-session isolation, deleted-fact non-recovery, reload, restart, and idempotent replay at `28 / 28` | [Official OpenClaw WebChat](evidence/2026-07-23-official-openclaw-webchat.md) | Qualifies one exact browser/runtime trajectory and observed free provider route, not model ranking, every channel, automatic governance, or transcript erasure. |
...
| OpenClaw | Conversation continuity, formation/review, verified tool outcomes, leased long-running operations with restart/reclaim fencing, correction, deletion, links, fail-open, the B03 same-run Web Chat/Hermes bridge through the official Gateway HTTP agent path, and W38 official Control UI recall/deletion/restart with real Grok | Messaging channels and arbitrary session migration beyond the qualified cases |
